...
class testZmqIfc( unittest.TestCase):
    ...
    def testExecHsh1( self) : 
        ...
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'command': ['cls', 'delete']})
        ...
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'putData': {'title': "Important Data", 
                                              'comment': "a comment", 
                                              'columns': 
        ...
        # retrieve the data 
        #
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'getData': True})
        self.assertEqual( hsh[ 'result'], 'done')
        #
        # ... and compare.
        #
    ...
    def testExecHsh2( self) : 
        ...
        d1 = [random.random() for n in range( MAX)]
        d2 = [random.random() for n in range( MAX)]
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'Scan': { 'name': "d1", 'x': pos, 'y': d1}})
        self.assertEqual( hsh[ 'result'], 'done')
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'Scan': { 'name': "d2", 'x': pos, 'y': d2}})
        self.assertEqual( hsh[ 'result'], 'done')

        PySpectra.misc.zmqIfc.execHsh( { 'command': ['setTitle \"a title\"']})
        self.assertEqual( hsh[ 'result'], 'done')
        PySpectra.misc.zmqIfc.execHsh( { 'command': ['setComment \"a comment\"']})
        self.assertEqual( hsh[ 'result'], 'done')
        PySpectra.misc.zmqIfc.execHsh( { 'command': ['display']})
        self.assertEqual( hsh[ 'result'], 'done')
        PySpectra.procEventsLoop( 1)
        ...
        #
        for i in range( MAX):
            PySpectra.misc.zmqIfc.execHsh( { 'command': ['setY d1 %d %g' % (i, float(i)/10.)]})
        #
        # and compare
        #
    ...
    def testExecHsh3( self) : 
        ...
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'command': ['cls', 'delete']})
        self.assertEqual( hsh[ 'result'], 'done')
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'command': ['setWsViewport dina5s']})
        self.assertEqual( hsh[ 'result'], 'done')
        ...
        for i in range(width):
            for j in range(height):
                ...
                hsh = PySpectra.misc.zmqIfc.execHsh( hsh)
                self.assertEqual( hsh[ 'result'], 'done')
            hsh =  PySpectra.misc.zmqIfc.execHsh( { 'command': ['cls', 'display']})
            self.assertEqual( hsh[ 'result'], 'done')
        PySpectra.procEventsLoop( 1)
    ...
    def testExecHsh6( self) : 
        ...
        hsh = PySpectra.misc.zmqIfc.execHsh( { 'putData': 
